...
// Nova action com estatísticas detalhadas - processamento em lotes de 100
export async function bulkUpsertEmployeesWithStatsAction(
  ...
  // Processar em lotes de 100 para balancear performance e confiabilidade
  const batchSize = 100
  let totalAdded = 0
  let totalUpdated = 0
  let totalErrors = 0
  ...
  for (let i = 0; i < employees.length; i += batchSize) {
    ...
    console.log(`Processing batch ${Math.floor(i/batchSize) + 1} with ${batch.length} employees`)

    try {
      // Buscar CPFs existentes para este lote
      const cpfsToCheck = batch.map(e => e.cpf)
      ...
      if (existingError) {
        console.error('Error fetching existing employees for batch:', existingError)
      ...
      // Contar quantos serão adicionados vs atualizados neste lote
      const willBeUpdated = batch.filter(e => existingCpfsSet.has(e.cpf)).length
      const willBeAdded = batch.length - willBeUpdated

      // Fazer upsert deste lote
      const { data, error } = await supabase
        .from('employees')
        .upsert(batch, {
          onConflict: 'cpf',
      ...
      if (error) {
        ...
        totalAdded += willBeAdded
        totalUpdated += willBeUpdated
        console.log(`Batch successful: added ${willBeAdded}, updated ${willBeUpdated}`)
      }
    } catch (batchError) {
      console.error('Exception in batch processing:', batchError)
      totalErrors += batch.length
    }
  }

  console.log(`Total results: added ${totalAdded}, updated ${totalUpdated}, errors ${totalErrors}`)
