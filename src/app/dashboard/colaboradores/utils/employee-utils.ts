...
export const generateEmployeeLabel = (employee: Employee): string => {
  ...
  if (employee.isInternal) {
    ...
    const storeLines = employee.store
      ? breakTextIntoLines(`Loja ${employee.store}`, 20)
      : [];
    const positionLines = employee.position
      ? breakTextIntoLines(employee.position, 20)
      : [];
    ...
    const topOffset = Math.max(
      20,
      Math.round((heightDots - totalContentHeight) / 2) - 50
    );

    // Gerar ZPL para as linhas do nome (vertically centered)
    const nameZpl = nameLines
      .map(
        (line, index) =>
    ...
    const storeZpl = storeLines
      .map((line, index) => {
        const zpl = `^FO${leftColumnX},${
          currentY + index * 45
        }^A0N,35,35^FD${line}^FS`;
        return zpl;
      })
      .join("\n");
    ...
    const positionZpl = positionLines
      .map((line, index) => {
        const zpl = `^FO${leftColumnX},${
          currentY + index * 45
        }^A0N,35,35^FD${line}^FS`;
    ...
    const nameLines = breakTextIntoLines(employee.name, 20);
    const roleLines = employee.role
      ? breakTextIntoLines(employee.role, 20)
      : [];
    ...
    const roleZpl = roleLines
      .map((line, index) => {
        const zpl = `^FO${leftColumnX},${
          roleY + index * 55
        }^A0N,55,55^FD${line}^FS`;
        return zpl;
      })
      .join("\n");
...
//   a.click()
// }
export const downloadAllEmployeeLabels = async (
  employees: Employee[]
): Promise<void> => {
  try {
    const zip = new JSZip();
    ...
    for (let i = 0; i < employees.length; i += batchSize) {
      ...
      batch.forEach((employee) => {
        const zpl = generateEmployeeLabel(employee);
        const fileName = `etiqueta_${employee.name.replace(
          /[^a-zA-Z0-9]/g,
          "_"
        )}.zpl`;
        zip.file(fileName, zpl);
 ...
 */
export const importEmployeesFromCSV = (
  file: File,
  ...
  reader.onload = (e) => {
    ...
    const newEmployees: Employee[] = lines
      .slice(1)
      .map((line) => {
...
export const createEmployeeOnDb = async (
...
export const updateEmployeeOnDb = async (
  id: string,
 ...
 */
export const importEmployeesFromCSVAsync = (
  file: File
): Promise<Omit<Employee, "id">[]> => {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const newEmployees = raw.filter(
          (e): e is Omit<Employee, "id"> => e !== null
        );
